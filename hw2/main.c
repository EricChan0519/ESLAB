...
#define SSID     "NasaMan"
#define PASSWORD "eric1111"

uint8_t RemoteIP[] = { 172, 20, 10, 3 };
#define RemotePORT	65432
	...
	while (1) {
		if (Socket != -1) {
			...
			if (ret == WIFI_STATUS_OK) {
				printf("WIFI_STATUS_OK is true\n");
				if (Datalen > 0) {
					printf("Datalen > 0 is true\n");
					RxData[Datalen] = 0;
					TERMOUT("Received: %s\n", RxData);
					// Get accelerometer data
//          BSP_ACCELERO_AccGetXYZ(pDataXYZ);
//          snprintf((char *)TxData, sizeof(TxData), "X: %d, Y: %d, Z: %d\n", pDataXYZ[0], pDataXYZ[1], pDataXYZ[2]);
...
//    HAL_UART_Transmit(&hDiscoUart, (uint8_t*)pDataXYZ, sizeof(pDataXYZ), HAL_MAX_DELAY);
		HAL_Delay(2000);
	}
